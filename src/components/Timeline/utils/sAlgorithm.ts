...
export class STimelineAlgorithm {
  ...
  private calculateOptimalLayout(totalNodes: number, drawWidth: number) {
    ...
    const optimalNodesPerRow = Math.min(
      this.maxNodesPerRow, 
      Math.max(minNodesPerRow, idealNodesPerRow)
    );
  ...
  generateSVGPath(milestones: MilestoneData[]): string {
    ...
    let pathData = '';
    const layout = this.calculateOptimalLayout(milestones.length, 
    ...
    for (let i = 0; i < milestones.length - 1; i++) {
      ...
      if (currentRow === nextRow) {
        ...
        const controlPoint1X = isCurrentRowEven ? 
          current.x + 50 : current.x - 50;
        const controlPoint2X = !isCurrentRowEven ? 
          next.x + 50 : next.x - 50;
